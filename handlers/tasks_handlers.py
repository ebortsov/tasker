...
from keyboards import keyboards, update_utc_keyboard
...
async def finish_task(message: Message, state: FSMContext, lexicon: DefaultLexicon):
    await message.answer(
        text=lexicon.msg_enter_task_description, reply_markup=ReplyKeyboardRemove()
    )
...
async def enter_description(
    ...
    await message.answer(
        text=lexicon.msg_task_completed.format(
            task_name=current_task.name, hours=hours, minutes=minutes
        ),
        reply_markup=keyboards.get_start_kb(lexicon),
    )
