...
GET_ALL_COMPLETED_TASK = (
    """SELECT rowid, * FROM history_of_users_tasks WHERE telegram_user_id = ?"""
)
EDIT_TASK_NAME = """
...
SELECT_TASK_BY_ID = (
    """SELECT rowid, * FROM history_of_users_tasks WHERE rowid = ? AND telegram_user_id = ?"""
)
...
def save_task(db_conn: sqlite3.Connection, task: Task):
    with db_conn:
        db_conn.execute(
            SAVE_TASK,
            (task.user_id, task.name, task.desc, task.start_time, task.end_time),
        )
...
def edit_task_description(
    db_conn: sqlite3.Connection, new_task_description: str, task_id: int, user_id: int
) -> None:
    with db_conn:
        db_conn.execute(EDIT_TASK_DESCRIPTION, (new_task_description, task_id, user_id))


def delete_task(db_conn: sqlite3.Connection, task_id: int, user_id: int) -> None:
    with db_conn:
        db_conn.execute(DELETE_TASK, (task_id, user_id))


def get_task(db_conn: sqlite3.Connection, task_id: str | int, user_id: int) -> Task | None:
    with db_conn:
